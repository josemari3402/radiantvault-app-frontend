...
import Nightmarket from './components/nightmarket'; // Strictly lowercase path[cite: 11]
import './App.css';
...
function App() {
  ...
  useEffect(() => {
    const fetchSkins = async () => {
      ...
      } catch (err) { console.error("Skin Sync Error", err); }
    };
    fetchSkins();
  }, []);
        ...
        <div className="landing-page" style={{ backgroundImage: `url(${bgImage})` }}>
          <div className="login-overlay">
            <div className="login-box">
              ...
              <button className="initialize-btn" onClick={handleAuth}>INITIALIZE</button>
              <button className="signup-toggle" onClick={() => setIsSigningUp(!isSigningUp)}>
                {isSigningUp ? "GO TO LOGIN" : "NEED ACCOUNT?"}
              </button>
            </div>
          </div>
        </div>
